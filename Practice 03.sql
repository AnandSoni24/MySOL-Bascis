...
/* Q5.List the Name of Employee who work on every Project? */
select ename from  emp where  not exists 
(select pno from   proj where  not exists 
(select * from   workson where  workson.eno = emp.eno and workson.pno =  proj.pno));
